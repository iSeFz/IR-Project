...
public class WebCrawlerWithDepth {
    ...
    public void getPageLinks(String URL, int depth, invertedIndex.Index5 index) throws IOException {
        System.out.println(
        ...
        if ((!(links.contains(URL)))
                ...
                && ((!URL.substring(12).contains(":")) || (depth == 0)) // ignor sublink that contain : bu pass the
                                                                        // "http:"
                && (!URL.isEmpty())
            ...
            try {
                ...
                // ??????------------------------------------------------------------------------
                // *** 3- get all paragraphs <p></p> elments from the page (document)
                // use document select with parameter "p"
                Elements paragraphContents = document.select("p");

                // ??????------------------------------------------------------------------------
                // **** 4- get the text inside those parargraphs inside the tags <p></p>
                // *** accumulate then into to String docText
                String docText = paragraphContents.text();

                // **** build the sources (given)
                SourceRecord sr = new SourceRecord(fid, URL, document.title(),
    ...
    public void parsePageLinks(String URL, int depth, invertedIndex.Index5 index) throws IOException {
    ...
    public invertedIndex.Index5 initialize(String storageName) throws IOException {
        invertedIndex.Index5 index = new invertedIndex.Index5();
        setSources(index);
    ...
    void setDomainKnowledge(invertedIndex.Index5 index, String domain) throws IOException {
